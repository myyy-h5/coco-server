...
const octokit = new Octokit({ auth: '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' });

function downloadFunc(downloadRepoUrl, temp_dest) {
  return new Promise(async (resolve, reject) => {
    console.log(downloadRepoUrl);
    download('myyy-h5/coco-template', temp_dest,  (err) => {
      if (err) {
        console.log(err);
        reject('请求模板下载失败');
      } else {
...
class ProjectService extends Service {
  async createProject(config) {
    ...
    const {data: {id, ssh_url}} = await octokit.request('POST /orgs/myyy-h5/repos', {
      org: 'myyy-h5',
      name: config.name
    });
